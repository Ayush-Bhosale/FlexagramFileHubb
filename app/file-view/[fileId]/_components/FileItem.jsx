...
const FileItem = ({ file }) => {
  ...
  return file && (
    <div className='h-[500px] bg-white rounded-lg flex flex-col items-center p-8'>
      <h1 className='text-xl'><strong className='text-violet-500'>{file?.userName}</strong> Shared file with You</h1>
  ...
  <div className='w-[300px] mt-5'>
    <input
      className='w-full border border-gray-500 outline-violet-500 text-base p-2 rounded-lg'
      type="password"
    ...
    />
      ...
      onClick={()=>{window.open(file?.fileUrl)}}
       disabled={file?.password && userPassword !== file?.password}
        className='bg-violet-500 disabled:bg-gray-500 disabled:cursor-not-allowed w-full flex justify-center items-center mt-5 p-2 rounded-full cursor-pointer text-white gap-2 text-lg active:bg-violet-600'>
        <Download />
        Download
      </button>
